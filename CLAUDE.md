...
- **No keywords** — symbol-based: `^` entry point, `<<` import, `>>` export, `|>` pipe (first-arg injection: `x |> f(a)` ≡ `f(x, a)`), `?`/`|`/`_` pattern matching, `? :` ternary, `~` comments. There is **no loop construct** — iterate with array methods (`.each`/`.map`/`.filter`/`.reduce`) and recursion (self-tail-calls are lowered to loops). Consult the symbol table in `docs/README.md`.
