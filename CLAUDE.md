...
- Sum types (`Ok`/`NotOk`) are tagged unions (i8 tag + payload). Payloads work end-to-end for `Num`/`Text`/`Bool`, and `Text` (or a nested array) inside a record/array or as a payload (`Ok(text)`) type-checks and runs via the codegen type-oracle side-table. Not yet unified: distinct payload *types* per slot across variants (a position that is `Num` in one variant and `Text` in another) — check `LANGUAGE.md` "Known limitations" and `tests/sum_*.rs` before assuming.
