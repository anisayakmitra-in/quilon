...
| `@read() -> Text` | Read one line from stdin (without the trailing newline). A [leaf IO primitive](#concurrency--colorless-implicit-futures--in-progress): it launches the read and returns a **deferred** `Text` forced on first strict use. Yields `""` at end-of-input. |
...
> in `core.time`), and — the *deferred value* half — the value-returning `@read` primitive
> (`@read() -> Text`, in `core.io`). `@read` launches a stdin line read and hands back a
...
**Runnable today (`@read`, a deferred value).** `@read() -> Text` (in `core.io`) reads one
...
^ = () -> Num => <
  ...
  assertEq(line, "hello")     ~ the comparison FORCES it: parks until ready, then reads bytes
  0
>
~ run as:  echo hello | quilon run examples/deferred_read.ql
```

`@read` and `^` and any helper are unmarked — only the leaf `@read` carries `@`. Binding
`line` does not wait; the force is the `==` inside `assertEq`. (Because `print`/`eprint`
...
| I/O: `@read` — deferred stdin line read, forced on use | ✅ |
