...
|------|--------|
| `quilon fmt` / linter | ⬜ |
| `quilon test` — in-language test framework (`describe`/`it`, pluggable reporters, blocks erased from every other command) | 🔨 (cases assert with the fail-fast `assert*`, so a run stops at the first failure; a report-every-failure matcher API is next) |
