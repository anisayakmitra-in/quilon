...
//! a `.ql` line and to step through source. Local-variable and full-type debug info is not
//! emitted yet — the subroutine type here is intentionally empty (no parameter/return types).
...
impl<'ctx> DebugInfo<'ctx> {
    ...
    pub fn create_function(&self, name: &str, span: &Span) -> DISubprogram<'ctx> {
        ...
        // Line tables only: an empty subroutine type (no parameter/return types).
        let subroutine_type =
            self.builder
